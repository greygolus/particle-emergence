...
import { exchangeDebris } from '../systems/debris';
...
function handleAction(action: string, element: HTMLElement): void {
  ...
  switch (action) {
    ...
    case 'run-collider':
      updateState(s => {
        if (s.collider.isBosonMode && s.colliderUpgrades.bosonModeUnlocked) {
          const check = canRunBosonCollider(s);
          if (!check.canRun) return s;
          const { newState } = runBosonCollider(s);
          return newState;
        } else {
          const check = canRunCollider(s, s.collider.tier);
          if (!check.canRun) return s;
          const { newState } = runCollider(s);
          return newState;
        }
      });
      break;
    ...
    // === DEBRIS ===
    case 'debris-to-pq':
      updateState(s => exchangeDebris(s, 'pq', Math.floor(s.debris)));
      break;
    case 'debris-to-pl':
      updateState(s => exchangeDebris(s, 'pl', Math.floor(s.debris)));
      break;
    case 'debris-to-energy':
      updateState(s => exchangeDebris(s, 'energy', Math.floor(s.debris)));
      break;
    case 'debris-to-pity':
      updateState(s => exchangeDebris(s, 'pity', Math.floor(s.debris)));
      break;
